...
async function analyzePatient(pid, pname) {
  ...
  modalBody.innerHTML = `
    <div class="ai-loading">
      <span class="pulse-dot"></span> Generating insights for ${pname}...
    </div>
  `;
  
  try {
    const response = await fetch(`/api/analyze/${pid}`);
    ...
    if (data.status === 'success') {
      ...
      modalBody.innerHTML = `<div class="ai-result"><p>${html}</p></div>`;
    } else {
      modalBody.innerHTML = `<div class="ai-error">❌ ${data.analysis}</div>`;
    }
  } catch (err) {
    modalBody.innerHTML = `<div class="ai-error">❌ Failed to reach AI service.</div>`;
    console.error(err);
  }
}
